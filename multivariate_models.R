...
fit_var_models <- function(ts_list, exog_ts_list = NULL, log_transform = LOG_TRANSFORM, 
  ...
  # Determine best lag order for VAR
  var_select <- tryCatch({
    # Use vars::VARselect to determine optimal lag order
    if (is.null(exog_data)) {
      vars::VARselect(model_data, lag.max = max_lags, type = "const")
    } else {
      vars::VARselect(model_data, lag.max = max_lags, type = "const", exogen = exog_data)
    }
  }, error = function(e) {
  ...
  # Extract optimal lag order based on the specified information criterion
  ...
  for (i in 0:(n_obs - rolling_window - forecast_horizon - optimal_lag)) {
    ...
    actual_values[[i+1]] <- model_data[test_start:test_end, ]
  ...
  error_metrics <- calculate_var_forecast_errors(forecasts, actual_values, colnames(model_data))
  ...
  for (model_name in model_names) {
    ...
    for (var_name in variable_names) {
      ...
      for (i in seq_along(forecasts)) {
        ...
        if (!(var_name %in% names(fc$fcst))) next
