...
aggregate_to_quarterly <- function(monthly_ts, aggregation_method = "average") {
  ...
  # Create data frame with values and time information
  monthly_df <- data.frame(
    value = as.numeric(monthly_ts),
    time = time(monthly_ts)
  )
  
  # Add year and month columns
  monthly_df$year <- floor(monthly_df$time)
  monthly_df$month <- round((monthly_df$time - monthly_df$year) * 12) + 1
  
  # Add quarter column
  monthly_df$quarter <- ceiling(monthly_df$month / 3)
  
  # Create yearquarter identifier (e.g., 2020.00, 2020.25, 2020.50, 2020.75)
  monthly_df$yearquarter <- monthly_df$year + (monthly_df$quarter - 1) / 4
  
  # Group by yearquarter and aggregate
  quarterly_df <- stats::aggregate(
    value ~ yearquarter, 
    data = monthly_df,
    FUN = agg_func
  )
  ...
  # Sort by time
  quarterly_df <- quarterly_df[order(quarterly_df$yearquarter), ]
  
  # Extract year and quarter from the first yearquarter value
  first_yearquarter <- quarterly_df$yearquarter[1]
  first_year <- floor(first_yearquarter)
  first_quarter <- round((first_yearquarter - first_year) * 4) + 1
  
  # Create quarterly time series
  quarterly_ts <- ts(
    quarterly_df$value,
    start = c(first_year, first_quarter),
    frequency = 4
  )
